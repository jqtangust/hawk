...
from hawk.common.dist_utils import get_rank, get_world_size, is_main_process, is_dist_avail_and_initialized
from hawk.common.logger import MetricLogger, SmoothedValue
from hawk.common.registry import registry
from hawk.datasets.data_utils import prepare_sample
...
class BaseTask:
    ...
    def _train_inner_loop(
        ...
        metric_logger.add_meter("totalloss", SmoothedValue(window_size=1, fmt="{value:.7f}"))
        ...
        log_freq_my = 10
        for i in metric_logger.log_every(range(iters_per_epoch), log_freq_my, header):
            # if using iter-based runner, we stop after iters_per_epoch iterations.
            if i >= iters_per_epoch:
                break
            
            logging.info("CURRENT ITER: %d" %i)
            ...
            metric_logger.update(totalloss=loss.item())
